...
function Header(){
	if($this->PageNo()==1){
		...
		$logo_empresa=crear_imagen($_SESSION['gt_bbdd']);
		
		if($this->CurOrientation == 'L'){
			$this->Image(RAIZ.'img/cabecera_informes.jpg',3,3,290,37);
			if($logo_empresa){
				$this->imagen_logo(RAIZ.'tmp/'.$logo_empresa,201,3,'','');
			}
		}
		
		if($this->CurOrientation == 'P'){
			$this->Image(RAIZ.'img/cabecera_informes.jpg',3,3,205,30);
			if($logo_empresa){
				$this->imagen_logo(RAIZ.'tmp/'.$logo_empresa,143,3,'','');
			}
		}
		if($logo_empresa){
			borrar_imagen($logo_empresa);
		}
	}
	$this->_makePageSize();
}

function titulo($x,$titulo){
	if($this->CurOrientation == 'L'){
		$this->SetFont('Arial','',12);
		$this->SetTextColor(255,255,255);
		$this->Text($x,38,$titulo);
	}
	
	if($this->CurOrientation == 'P'){
		$this->SetFont('Arial','',12);
		$this->SetTextColor(255,255,255);
		$this->Text($x,31,$titulo);
	}
}

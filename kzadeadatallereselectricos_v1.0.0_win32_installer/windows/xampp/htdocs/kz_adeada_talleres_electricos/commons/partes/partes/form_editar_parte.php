...
<form action='index.php?menu=administracion_partes' method='POST'>
		...
		<tr>
			...
			<?php if($_POST['pag'] == '../commons/partes/partes'){?>
				<select name="cliente_proyecto" id="cliente_proyecto">
	         		<?php
	         		$cliente_proyecto=select_normal("SELECT c.id as idcliente, c.nombre_comercial, p.id as idproyecto, p.nombre FROM kz_te_clientes c, kz_te_proyectos p WHERE c.id = p.cliente ORDER BY c.nombre_comercial");
	         		?><option></option><?php
	...
	<input type='hidden' name='editar_parte' value='true'></input>

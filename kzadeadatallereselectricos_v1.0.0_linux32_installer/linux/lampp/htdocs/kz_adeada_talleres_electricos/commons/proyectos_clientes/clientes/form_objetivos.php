...
?>
<?php
$annos = select_normal("SELECT distinct anno FROM kz_te_objetivos_clientes WHERE cliente = '".$_POST['codigo']."' ORDER BY anno");
?>

<br>
<fieldset>
	<legend><b>Objetivos para el cliente <?php echo $_POST['codigo']; ?></b></legend>
	<?php if($annos){ ?>
		<table class="tabla_sin_borde">
			<?php foreach ($annos as $key => $valor){ ?>
				<tr>
					<td>
						<?php echo "<b>".$valor['anno'].":</b>"; ?>
					</td>
				</tr>
				
				<?php 
				$objetivos = select_normal("SELECT * FROM kz_te_objetivos_clientes WHERE cliente = '".$_POST['codigo']."' and anno = '".$valor['anno']."'");
				...
				<tr>
					...
					<td>
						<form action='index.php?menu=administracion_proyectos' method='POST'>
							...
							<input type='hidden' name='codigo' value='<?php  echo $valor['cliente']; ?>' />
